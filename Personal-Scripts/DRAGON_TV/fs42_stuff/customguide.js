...
const SIDE_IMAGE_OFFSET_Y = parseInt(params.get('side_image_offset') || '0', 10);
const SIDE_IMAGE = params.get('side_image') || 'https://www.veltron.net/images/site-banner.png';
...
function buildRightImagePanel() {
  const panel = document.createElement('div');
  panel.id = 'right-image-panel';

  const img = document.createElement('img');
  img.id = 'guide-side-image';
  img.src = SIDE_IMAGE;
  img.alt = 'Side Panel';

  // 👇 apply vertical offset (in px)
  img.style.transform = `translateY(${SIDE_IMAGE_OFFSET_Y}px)`;
...
function buildGridDOM() {
  ...
  listings.innerHTML = `
    <div id="right-image-panel">
      <img id="guide-side-image" src="${SIDE_IMAGE}" alt="Side Panel" />
    </div>
    ...
    <div id="grid-wrapper">
      <div id="grid-header">
        <div class="grid-channel-stub"><span id="clock"></span></div>
      </div>
      <div id="grid-listings"></div>
    </div>
  `;

  // ✅ NOW DOM exists, so apply offset here
  const sideImg = document.getElementById('guide-side-image');
  if (sideImg) {
    sideImg.style.transform = `translateY(${SIDE_IMAGE_OFFSET_Y}px)`;
  }
